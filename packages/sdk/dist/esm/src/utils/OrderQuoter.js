...
import { OrderType, PERMIT2_MAPPING, REACTOR_ADDRESS_MAPPING, UNISWAPX_ORDER_QUOTER_MAPPING, UNISWAPX_V4_ORDER_QUOTER_MAPPING, } from "../constants";
import { OrderQuoter__factory, OrderQuoterV4__factory, RelayOrderReactor__factory, } from "../contracts";
                        ...
                        if (returnData.includes(key)) {
                            ...
                            if (key === "0a0b0d79") {
                                ...
                                if ("additionalValidationData" in orders[idx].order.info) {
                                    const fillerValidation = parseExclusiveFillerData(orders[idx].order.info
...
function decodeV4RevertData(returnData) {
    ...
    try {
        ...
        if (raw.startsWith(BASIC_ERROR)) {
            const decoded = new ethers.utils.AbiCoder().decode(["string"], "0x" + raw.slice(10))[0];
            return { raw, text: decoded };
        }
        // Panic(uint256)
        if (raw.startsWith(V4_PANIC_ERROR)) {
            const code = new ethers.utils.AbiCoder().decode(["uint256"], "0x" + raw.slice(10))[0];
...
export class V4OrderQuoter {
    ...
    getMulticallResults(provider, functionName, orders) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const promises = [];
            ordersWithBlockOverrides.map((order) => {
                promises.push(multicallSameContractManyFunctions(provider, {
                    address: this.quoter.address,
                    contractInterface: this.quoter.interface,
                    functionName: functionName,
                    functionParams: [
