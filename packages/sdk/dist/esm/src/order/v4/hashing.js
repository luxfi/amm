 ...
 */
export const ORDER_INFO_V4_TYPE_STRING = "OrderInfo(" +
    "address reactor," +
    "address swapper," +
    "uint256 nonce," +
    "uint256 deadline," +
 ...
 */
const HYBRID_ORDER_TYPE_HASH = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(HYBRID_ORDER_TYPE_STRING +
 ...
 */
export function hashOrderInfoV4(info) {
...
function hashHybridInput(input) {
    return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["bytes32", "address", "uint256"], [HYBRID_INPUT_TYPE_HASH, input.token, input.maxAmount]));
}
/**
 * Hash HybridOutput structure
 */
function hashHybridOutput(output) {
    return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["bytes32", "address", "uint256", "address"], [
...
export function hashHybridOrder(order) {
 ...
 */
export function hashHybridCosignerData(orderHash, cosignerData, chainId) {
    var _a;
    const encodedCosignerData = ethers.utils.defaultAbiCoder.encode([
        ...
        [
            ...
            cosignerData.supplementalPriceCurve.map((value) => BigNumber.from(value)),
            cosignerData.exclusiveFiller,
            BigNumber.from(cosignerData.exclusivityOverrideBps),
            BigNumber.from((_a = cosignerData.exclusivityEndBlock) !== null && _a !== void 0 ? _a : 0),
        ],
    ]);
    return ethers.utils.keccak256(ethers.utils.solidityPack(["bytes32", "uint256", "bytes"], [orderHash, chainId, encodedCosignerData]));
}
/**
...
function hashFeedInfo(feed) {
    return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["bytes32", "bytes32", "address", "string"], [FEED_INFO_TYPE_HASH, feed.feedId, feed.feed_address, feed.feedType]));
}
/**
 ...
 * @returns The keccak256 hash
 */
export function hashPrivateIntent(privateIntent) {
    const oracleFeedsHash = hashFeedInfoArray(privateIntent.oracleFeeds);
    return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["bytes32", "uint256", "uint256", "uint256", "bytes32", "bytes32"], [
...
function hashOutputAllocation(allocation) {
    return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["bytes32", "address", "uint16"], [
 ...
 * @returns The keccak256 hash
 */
export function hashDCAIntent(intent, privateIntentHash) {
 ...
 * @returns The keccak256 hash
 */
export function hashDCACosignerData(cosignerData) {
    return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["bytes32", "address", "uint96", "uint160", "uint96", "uint160"], [
